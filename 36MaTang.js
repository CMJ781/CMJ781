 ...
 */
let config = {
    darksky_api: "851f6b96816b3d8b69d691e5e1bf1dea", //从https://darksky.net/dev/ 上申请key填入即可
    aqicn_api: "992aa885b20b17574645f8cdd4c0cf75280d0d87", //从http://aqicn.org/data-platform/token/#/ 上申请key填入即可
    huweather_apiKey: "9914671b84ec4ee096cd2cc440ea173a", //和风天气APIkey,可自行前往 https://dev.heweather.com/ 进行获取
    lat_lon: "32.377622, 121.057970", //请填写经纬度,直接从谷歌地图中获取即可
...
const provider = {
    heweather_now: {
        api: `https://api.heweather.net/s6/weather/now?location=${config.lat_lon.replace(/\s/g, "").replace("，", ",")}&key=${config.huweather_apiKey}`,
    ...
    heweather_daily: {
        api: `https://api.heweather.net/s6/weather/forecast?location=${config.lat_lon.replace(/\s/g, "").replace("，", ",")}&key=${config.huweather_apiKey}`,
        progress: 0,
        timeoutNumber: 0,
        data: {},
    ...
    heweather_air: {
        api: `https://api.heweather.net/s6/air/now?location=auto_ip&key=${config.huweather_apiKey}`,
        progress: 0,
        timeoutNumber: 0,
        data: {},
        support: []
    ...
    heweather_lifestyle: {
        api: `https://api.heweather.net/s6/weather/lifestyle?location=${config.lat_lon.replace(/\s/g, "").replace("，", ",")}&key=${config.huweather_apiKey}`,
        progress: 0,
        timeoutNumber: 0,
        data: [],
        support: ['$[lifeStyle]']
